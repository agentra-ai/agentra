...
This guide documents the local development workflow for contributors working on the Agentra codebase.
...
```bash
git worktree add ../agentra-feature -b feat/my-change main
cd ../agentra-feature
make worktree-env
make setup-worktree
make start-worktree
...
- main checkout
  ...
  - frontend: `3000`
- worktree checkout
  - database: `agentra_my_feature_702`
...
The daemon authenticates using the CLI's stored token (`agentra login`).
...
```bash
docker compose exec -T postgres psql -U agentra -d postgres -At -c "select datname from pg_database order by datname;"
```
