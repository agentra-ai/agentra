...
UPDATE inbox_item SET archived = true
WHERE id = $1
RETURNING id, workspace_id, recipient_type, recipient_id, type, severity, issue_id, title, body, read, archived, created_at, actor_type, actor_id, details, lifecycle_event_id
`
...
const createInboxItemForLifecycleEvent = `-- name: CreateInboxItemForLifecycleEvent :one
INSERT INTO inbox_item (
    workspace_id, recipient_type, recipient_id,
    type, severity, issue_id, title, body,
...
type CreateInboxItemForLifecycleEventParams struct {
	WorkspaceID      pgtype.UUID `json:"workspace_id"`
	RecipientType    string      `json:"recipient_type"`
	RecipientID      pgtype.UUID `json:"recipient_id"`
	Type             string      `json:"type"`
...
func (q *Queries) CreateInboxItemForLifecycleEvent(ctx context.Context, arg CreateInboxItemForLifecycleEventParams) (InboxItem, error) {
	row := q.db.QueryRow(ctx, createInboxItemForLifecycleEvent,
		arg.WorkspaceID,
		arg.RecipientType,
		arg.RecipientID,
		arg.Type,
...
SELECT id, workspace_id, recipient_type, recipient_id, type, severity, issue_id, title, body, read, archived, created_at, actor_type, actor_id, details, lifecycle_event_id FROM inbox_item
WHERE id = $1
`
...
SELECT i.id, i.workspace_id, i.recipient_type, i.recipient_id, i.type, i.severity, i.issue_id, i.title, i.body, i.read, i.archived, i.created_at, i.actor_type, i.actor_id, i.details, i.lifecycle_event_id,
