...
var authLoginCmd = &cobra.Command{
	Use:    "login",
	Short:  "Authenticate with Agentra",
	Long:   "Authenticate with Agentra without auto-configuring workspaces. Use 'agentra login' for the guided setup flow.",
	Hidden: true,
	RunE:   runAuthLogin,
}
...
func resolveToken(cmd *cobra.Command) string {
	if v := strings.TrimSpace(os.Getenv("AGENTRA_TOKEN")); v != "" {
		return v
	}
...
func runAuthStatus(cmd *cobra.Command, _ []string) error {
	...
	if token == "" {
		fmt.Fprintln(os.Stderr, "Not authenticated. Run 'agentra login' to authenticate.")
		return nil
	}
...
<title>Agentra — Authenticated</title>
<style>
