...
func newAPIClient(cmd *cobra.Command) (*cli.APIClient, error) {
	...
	if serverURL == "" {
		return nil, fmt.Errorf("server URL not set: use --server-url flag, AGENTRA_SERVER_URL env, or 'agentra config set server_url <url>'")
	}

	client := cli.NewAPIClient(serverURL, workspaceID, token)
	// When running inside a daemon task, attribute actions to the agent.
	if agentID := os.Getenv("AGENTRA_AGENT_ID"); agentID != "" {
		client.AgentID = agentID
	}
	if taskID := os.Getenv("AGENTRA_TASK_ID"); taskID != "" {
...
func resolveServerURL(cmd *cobra.Command) string {
	...
	if v := strings.TrimSpace(os.Getenv("AGENTRA_CLI_SERVER_URL")); v != "" {
		return normalizeAPIBaseURL(v)
	}
	if v := strings.TrimSpace(os.Getenv("AGENTRA_SERVER_URL")); v != "" {
		return normalizeAPIBaseURL(v)
	}
...
func resolveWorkspaceID(cmd *cobra.Command) string {
	val := cli.FlagOrEnv(cmd, "workspace-id", "AGENTRA_WORKSPACE_ID", "")
	if val != "" {
		return val
	}
	profile := resolveProfile(cmd)
