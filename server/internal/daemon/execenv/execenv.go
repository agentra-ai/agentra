...
// checked out on demand by the agent via `agentra repo checkout`.
package execenv
...
type PrepareParams struct {
	WorkspacesRoot string           // base path for all envs (e.g., ~/agentra_workspaces)
...
type TaskContextForEnv struct {
	IssueID           string
	IssueTitle        string // issue title for branch naming
...
// on demand via `agentra repo checkout <url>`.
