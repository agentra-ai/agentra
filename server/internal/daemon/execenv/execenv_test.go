...
func TestInjectRuntimeConfigClaude(t *testing.T) {
	...
	s := string(content)
	for _, want := range []string{
		"Agentra Agent Runtime",
		"agentra issue get",
		"agentra issue comment list",
		"Go Conventions",
		"PR Review",
...
func TestInjectRuntimeConfigCodex(t *testing.T) {
	...
	s := string(content)
	if !strings.Contains(s, "Agentra Agent Runtime") {
...
func TestInjectRuntimeConfigNoSkills(t *testing.T) {
	...
	s := string(content)
	if !strings.Contains(s, "agentra issue get") {
		t.Error("should reference agentra CLI even without skills")
	}
