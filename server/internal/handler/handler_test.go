...
const (
	handlerTestEmail         = "handler-test@agentra.ai"
...
func TestSendCode(t *testing.T) {
	w := httptest.NewRecorder()
	body := map[string]string{"email": "sendcode-test@agentra.ai"}
	var buf bytes.Buffer
	json.NewEncoder(&buf).Encode(body)
	...
	if resp.DevCode == nil || *resp.DevCode == "" {
		t.Fatal("SendCode: expected dev_code in development mode")
	}

	t.Cleanup(func() {
		testPool.Exec(context.Background(), `DELETE FROM verification_code WHERE email = $1`, "sendcode-test@agentra.ai")
	})
}

func TestSendCodeRateLimit(t *testing.T) {
	const email = "ratelimit-test@agentra.ai"
	t.Cleanup(func() {
...
func TestVerifyCodeWrongCode(t *testing.T) {
	const email = "wrong-code-test@agentra.ai"
	ctx := context.Background()
...
func TestVerifyCodeCreatesWorkspace(t *testing.T) {
	const email = "workspace-verify-test@agentra.ai"
	ctx := context.Background()
