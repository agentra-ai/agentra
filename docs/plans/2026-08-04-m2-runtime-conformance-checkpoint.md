...
| M2A 剩余实现 | 可推进 | usage/artifact、UI capability 回归 | 在本地实现并测试 | 否 |
...
1. 本地完成 usage/artifact fixture，补齐 M2A 剩余契约缺口；crash-resume 已有 adapter、API、数据库恢复和隔离回归。
2. 完成 Web capability 展示和拒绝路径，使用户在 dispatch 前看到限制。
...
当前判定是 **in progress**。下一项是 usage/artifact fixture；推送不是这一步的
前置条件。
