...
| M2A 剩余实现 | 可推进 | 通用 pre-claim capability 回归 | 在本地实现并测试 | 否 |
...
1. 完成通用 pre-claim capability 过滤，避免不兼容任务先 claim 再失败；Web capability 展示、crash-resume 与 usage/artifact wire contract 已有 API、UI、数据库和隔离回归。
...
当前判定是 **in progress**。下一项是通用 pre-claim capability 过滤；推送不是这一步的
前置条件。
