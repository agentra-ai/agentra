...
终态记录，不代表当前工作不可推进。真正未解除的门槛只有两类：尚未完成的本地实现，
以及需要把提交送到托管环境后才能取得的外部证据。
...
| M2A 剩余实现 | 可推进 | UI capability 与通用 pre-claim 回归 | 在本地实现并测试 | 否 |
...
“外部门槛”不等于整个目标受阻：只有当本地门槛全部完成、且后续动作只剩外部授权时，
M2A 才进入真正的 blocked 状态。
...
1. 完成 Web capability 展示和拒绝路径，使用户在 dispatch 前看到限制；crash-resume 与 usage/artifact wire contract 已有 adapter、API、数据库和隔离回归。
2. 完成通用 pre-claim capability 过滤，避免不兼容任务先 claim 再失败。
3. 再跑 `make check`，保持本地门槛为绿色。
4. 经用户授权后推送当前提交，取得三平台托管 CI 证据并修复平台差异。
5. 在隔离环境运行真实 provider smoke，记录版本、能力和偏差。
6. 证据齐全后关闭 M2A；随后分别开启 M2B（隔离与 SDK）和 M2C（provider 扩展）。
...
当前判定是 **in progress**。下一项是 UI capability 展示与通用 pre-claim 过滤；推送不是这一步的
前置条件。
