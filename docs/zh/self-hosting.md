...
bootstrap 将 PostgreSQL/JWT/MinIO 随机密钥写入仅所有者可访问的 `.env`，并拒绝覆盖已有文件。默认栈只在 loopback 发布前后端；PostgreSQL 与 MinIO 只留在 Compose 网络，Adminer（`debug`）和挂载 Docker socket 的 gateway（`cloud-runtime`）必须显式启用 profile。
