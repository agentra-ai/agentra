...
export function WorkspaceTab() {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      toast.error(e instanceof Error ? e.message : t("settingsSaveFailed"));
    } finally {
      setSaving(false);
    }
  };
  ...
  const handleLeaveWorkspace = () => {
    ...
    setConfirmAction({
      ...
      description: t("leaveWarningBody"),
      variant: "destructive",
      onConfirm: async () => {
        setActionId("leave");
        try {
          ...
          toast.error(e instanceof Error ? e.message : t("leaveFailed"));
        } finally {
          setActionId(null);
        }
      },
  ...
  const handleDeleteWorkspace = () => {
    ...
    setConfirmAction({
      ...
      description: t("deleteWarningBody"),
      variant: "destructive",
      onConfirm: async () => {
      ...
      <section className="space-y-4">
        ...
        <Card>
          <CardContent className="space-y-3">
            ...
            <div>
              <Label className="text-xs text-muted-foreground">{t("workspaceFocus")}</Label>
              <Textarea
                value={description}
                ...
                placeholder={t("workspaceFocus")}
              />
            </div>
            <div>
              <Label className="text-xs text-muted-foreground">{t("workspaceFocusDescription")}</Label>
              <Textarea
                value={context}
                ...
                disabled={!canManageWorkspace}
                className="mt-1 resize-none"
                placeholder={t("workspaceFocusDescription")}
              />
            </div>
            <div>
        ...
        <Card>
          <CardContent className="space-y-3">
            <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
              ...
              >
                {actionId === "leave" ? tc("loading") : tc("confirm")}
              </Button>
            </div>
            ...
            {isOwner && (
              <div className="flex flex-col gap-2 border-t pt-3 sm:flex-row sm:items-center sm:justify-between">
                ...
                >
                  {actionId === "delete-workspace" ? tc("loading") : tc("delete")}
                </Button>
              </div>
            )}
          </CardContent>
